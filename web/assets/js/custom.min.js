...
$(document).ready(function() {
    ...
    console.log($('.ajaxCategoryAdd').length);
    // Make Post Request to add a new Category
    // $('body').on('submit', '.ajaxCategoryAdd', function (e) {
    ...
    //                 $('.modal').modal('hide');
    //                 $(this).reset();
    //                 // Append to list based on weight
    //             }
    //         }).fail(function (jqXHR, textStatus, errorThrown) {
    //             if (typeof jqXHR.responseJSON !== 'undefined') {
    //                 if (jqXHR.responseJSON.hasOwnProperty('errors')) {
    //                     $('.ajaxCategoryAdd .errors').html(jqXHR.responseJSON.errors);
    //                     var errors = jqXHR.responseJSON.errors;
    //                     for (var i = 0; i < errors.length; i++) {
    //
    //                         $('.bs-callout-warning').append("<p>" + errors[i] + "</p>")
    //                     }
        ...
        template: `<div>
             <ul class="catalog-categories list-unstyled msg_list">
                ...
                <category-item v-for="category in categories" :key="category.id">
                    <template slot="edit-link"><a :href="'/backend/catalog/ajaxCategoryEdit/' + category.id"></template>
                    <template slot="category-title">{{category.title}} - {{category.weight}}</template>
    ...
    var app = new Vue({
        ...
        data: {
            message: 'Hello Vue!',
            listJson: [],
            ajaxCategoryAdd: $('.ajaxCategoryAdd')
        },
        methods: {
            ...
            saveData: function () {
                ...
                var that = this;
                var url = $('.ajaxCategoryAdd').attr('action');

                axios.post(
                    url, $('.ajaxCategoryAdd').serialize(),
                    config
                )
                .then(function (response) {
                    Event.$emit('updateCategories', JSON.parse(response.data.category));
                    $('.modal').modal('hide');
                    $('.ajaxCategoryAdd').trigger('reset');
